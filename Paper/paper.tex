...
\begin{enumerate}[leftmargin=1.4em,itemsep=1pt]
    ...
    \item We conduct a checksum-bound GPT-2/Wikitext-2 study with eight prespecified primary seeds, exact-cost and common-random-number controls, allocation-only adaptive baselines, and a separate three-seed module-family boundary suite; the primary gain is statistically resolved, but the boundary suite reverses direction.
\end{enumerate}
...
The six paper-facing strategies are exact-cost uniform rank 4, activation-whitened spectral effective rank, gradient norm, and allocation-only EVA-, GoRA-, and FIM-LoRA-style controls.  The last three borrow only a scoring family from the corresponding method; they are not full reproductions of those methods' initialization or training algorithms.  The prespecified primary suite adapts GPT-2 \texttt{c\_attn} and \texttt{c\_fc} modules over eight independent base seeds.  A separate three-seed boundary suite adds \texttt{attn.c\_proj}; it is not pooled with the primary analysis.
...
In the prespecified $c_{attn},c_{fc}$ primary suite, spectral-effective allocation has mean final validation loss $3.4513$ (SEM $0.0010$), versus $3.4583$ ($0.0012$) for exact-cost uniform rank 4.  The paired mean difference is $-0.007026$ with bootstrap 95\% interval $[-0.008343,-0.006026]$, and spectral allocation wins all eight seeds.  The exact two-sided sign-flip test gives $p=0.0078125$; adjustment across the five prespecified within-suite candidate comparisons gives Holm $p=0.0390625$.  EVA-style allocation is also better than uniform in all eight seeds ($-0.004610$, interval $[-0.004911,-0.004315]$), but spectral allocation has the lower mean loss.  GoRA-style, gradient-norm, and FIM-LoRA-style allocations are worse than uniform by $+0.002683$, $+0.027467$, and $+0.041326$, respectively, under this common protocol.
